...
with col3:
    ...
    agency = st.selectbox('Agency', (options_agency), index=6)
...
#--- API key ---#
with col6:
    api_key = st.text_input('Demo API Key', value='YOUR_API_KEY', help='Register for an API key at https://www.dataportal.dol.gov/registration. If you are already registered, sign-in to the open data portal first and then get your key at https://www.dataportal.dol.gov/api-keys.')
...
if api_key in ['','YOUR_API_KEY']:
    ...
    st.markdown('<div class="warning-box">⚠️ <strong>Warning:</strong> You are missing an API key! Enter your API key to use the DOL API Explorer. </div>', unsafe_allow_html=True)
    ...
    try:
        # Proceed with API call only if API key is valid.
        metadata = get_metadata(agency=agency, endpoint=dataset, api_key=api_key)
    except Exception as e:
        st.error(f"An error occurred while fetching metadata: {e}")    
        metadata = None
